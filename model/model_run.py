...
def config():
 ...
 #   args.dataset_path = '/home/zheng/RatingTopicGraph/filtered_yelp_reviews_output/filtered_yelp_restaurant_reviews_output.jsonl' # 修改为您的 JSONL 文件路径
    args.dataset_name = 'filtered_Industrial_and_Scientific_output'
    args.dataset_path = '/data/cxf2022/dl_project/AQUARIUS-main/dataset/filtered_Industrial_and_Scientific_output.jsonl' # 修改为您的 JSONL 文件路径
...
class SentenceRetrival(nn.Module):
    ...
    def calc_ranking_loss(self, edges):
        ...
        # 添加数值稳定性：使用 softplus 替代 -sigmoid().log()
        # 原始: loss = - (pos_score - neg_score).sigmoid().log()
        # 使用 softplus: -log(sigmoid(x)) = softplus(-x) = log(1 + exp(-x))
...
class Net(nn.Module):
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        for name, param in self.named_parameters():
            if name not in state_dict:
                if 'review_embedding' in name or 'sentence_embedding' in name:
    ...
    @torch.no_grad()
    def evaluate_rating(self, dataloader, etype='valid', dataset_name='验证集'):
        ...
        for input_nodes, edge_subgraph, blocks in dataloader:
            ...
            # 清理GPU内存
            del input_nodes, edge_subgraph, blocks, p_ratings, true_ratings, rmse, mae
            torch.cuda.empty_cache()

        # 对每个评分组计算均值（即 MSE）
        group_mse = {group: np.mean(errors) for group, errors in group_errors.items()}
        # 输出各评分组的 MSE
        print(f"\n【{dataset_name}】各评分组（1～5）的均方误差（MSE）：")
        for group in sorted(group_mse.keys()):
            print(f"  评分 {group}: MSE = {group_mse[group]:.4f}")
    ...
    @torch.no_grad()
    def evaluate_sentence_ranking(self, dataloader, raw_graph, sampler, etype='valid', topk=5, dataset_name='验证集'):
        device = self.review_embedding.weight.device
        ...
        for rating_input_nodes, decoder_graph, rating_encoder_blocks in dataloader:
            ...
            # 清理GPU内存
            del rating_input_nodes, input_nodes, blocks, rating_encoder_blocks, decoder_graph
            del urf, irf, utf, itf, ranking_scores
            torch.cuda.empty_cache()
        ...
        # 输出每个评分组的排序指标
        print(f"\n【{dataset_name}】各评分组（1～5）的排序指标：")
        for group in sorted(group_metrics.keys()):
            metrics = group_metrics[group]
            print(f"  评分组 {group}: Pre = {metrics.get('Pre', 0):.4f}, Rec = {metrics.get('Rec', 0):.4f}, F1 = {metrics.get('F1', 0):.4f}, nDCG = {metrics.get('nDCG', 0):.4f}")
...
def train(params):
    ...
    # 训练前初始评估（使用验证集，避免信息泄露）
    logger.info('【初始评估】训练前的模型表现（验证集）：')
    net.evaluate_sentence_ranking(valid_dataloader, graph, topic_sampler, etype='valid', dataset_name='验证集')

    logger.info("Start training ...")
    for iter_idx in range(1, params.epoch):
        net.train()

        pbar = tqdm(train_dataloader, desc=f"Epoch {iter_idx}", ncols=100, leave=True)
        train_rmse = []
        train_mi = []
        for rating_input_nodes, edge_subgraph, rating_blocks in pbar:
            ...
            pbar.set_postfix(loss=f"{r_loss:.4f}", MI=f"{mi_score:.2f}")

            # 保存当前epoch的最后一批次的loss和mi_score
            last_r_loss = r_loss.item()
            last_mi_score = mi_score.item()

            # 清理GPU内存，避免OOM
            del rating_input_nodes, topic_input_nodes, edge_subgraph, rating_blocks, topic_blocks
            del r_loss, mi_score, ranking_loss, loss
            torch.cuda.empty_cache()
        ...
        train_rmse = last_r_loss
        train_mi = last_mi_score

        # 验证集评估
        print(f"\n{'='*60}")
        print(f"Epoch {iter_idx} 评估结果")
        print(f"{'='*60}")
        valid_rmse, valid_mae, valid_mse = net.evaluate_rating(valid_dataloader, etype='valid', dataset_name='验证集')
        logging_str = f"Epoch={iter_idx:>3d}, " \
                      f"Train_Loss={train_rmse:.4f}, MI={train_mi:.2f}, " \
                      f"Valid_RMSE={valid_rmse:.4f}, Valid_MAE={valid_mae:.4f}, Valid_MSE={valid_mse:.4f}"
    ...
    # 在测试集上评估一次
    test_rmse, test_mae, test_mse = net.evaluate_rating(test_dataloader, etype='test', dataset_name='测试集')
    ...
    print(f"\n【测试集】句子检索指标：")
    for k in [10, 50]:
        scores = net.evaluate_sentence_ranking(test_dataloader, graph, topic_sampler, etype='test', topk=k, dataset_name='测试集')
        print(f"  Top-{k}: Pre={scores['Pre']:.4f}, Rec={scores['Rec']:.4f}, F1={scores['F1']:.4f}, nDCG={scores['nDCG']:.4f}")

    logger.info(f'Best Iter Idx={best_iter}, Best Valid RMSE={best_valid_rmse:.4f}')
    logger.info(f'Final Test RMSE={test_rmse:.4f}, MAE={test_mae:.4f}, MSE={test_mse:.4f}')
...
def test(params):
    ...
    print(f"\n{'='*60}")
    print(f"【最终测试】数据集: {params.dataset_name}")
    print(f"{'='*60}")
